...
if (isset($_POST["guardar"])) {
  ...
  if ($file[0] > 0) {
    $direc = "documentos_IND/" . $id_contrato;
    ...
    foreach ($_FILES['adjuntar']['tmp_name'] as $key => $value ) {
      ...
      copy($_FILES['adjuntar']['tmp_name'][$key],
        "documentos_IND/" . $id_contrato . "/" . $_FILES['adjuntar']['name'][$key]);

      $url = "documentos_IND/".$id_contrato."/".$_FILES['adjuntar']['name'][$key];

      mysql_query("INSERT INTO documento_IND(id_contrato, documento, url)
...
if (isset($_REQUEST["eliminar"])) {
  ...
  $query = mysql_query("DELETE FROM documento_IND WHERE id_documento = '$id_eliminar' ") or die (mysql_error());
  }
...
<table id="table1" class="tabla display" cellspacing="0" width="100%">
  ...
  <tbody>
<?php
$query = mysql_query("SELECT id_documento, url, documento FROM documento_IND WHERE id_contrato = $id_contrato");
if (mysql_num_rows($query) > 0) {
  while ($row = mysql_fetch_array($query)) {
?>
    <tr>
      <td><a href=" <?php echo $row['url']; ?>"><?php echo $row['documento']; ?></a> <a href="ver-documento-IND.php?key=<?php echo $id_contrato; ?>&eliminar=<?php echo $row['id_documento']; ?>"><span class="glyphicon glyphicon-remove"></a>
      </td>
    </tr>
<?php 
    }
...
<div class="formulario pull-center">
  <button type="button" class="btn btn-primary" onclick="javascript:document.location.href = 'IND.php'"><span class="glyphicon glyphicon-repeat"></span> Volver</button>
</div>
